...
import molab.main.java.entity.Visitrecord;
 ...
 * 
 * @see molab.main.java.entity.Visitrecord
...
public class VisitrecordDAO extends BaseHibernateDAO {
	...
	public Visitrecord findById(java.lang.Integer id) {
		...
		try {
			Visitrecord instance = (Visitrecord) hibernateTemplate.get(
					"molab.main.java.entity.Visitrecord", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findCountMacAndAvgDwelltime(String monid, int start, int finish, int dwellTime) {
		log.debug("finding count(mac), avg(dwellTime) from Visitrecord with monid: " + 
				monid + ", inTime: " + start + ", dwellTime: " + dwellTime);
		try {
			String queryString = "select count(mac), avg(dwellTime) from Visitrecord where monid=? AND inTime between ? and ? AND dwellTime>?";
			return hibernateTemplate.find(queryString, monid, start, finish, dwellTime);
		} catch (RuntimeException re) {
			log.error("find all failed", re);
			throw re;
		}
	...
	public List findCountMacAndAvgDwelltimeForLarge(String[] monidList, int start, int finish, int dwellTime) {
		log.debug("finding count(mac), avg(dwellTime) from Visitrecord with monid: " + 
				monidList + ", inTime: " + start + ", dwellTime: " + dwellTime);
		try {
			String queryString = "select count(mac), avg(dwellTime) from Visitrecord where monid in (:monidListParam) AND inTime between :startParam and :finishParam AND dwellTime>:dwellTimeParam";
			String[] params = {"monidListParam", "startParam", "finishParam", "dwellTimeParam"};
			Object[] values = {monidList, start, finish, dwellTime};
