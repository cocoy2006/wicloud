...
import molab.main.java.entity.Branddis;
 ...
 * 
 * @see molab.main.java.entity.Branddis
...
public class BranddisDAO extends BaseHibernateDAO {
	...
	public Branddis findById(molab.main.java.entity.BranddisId id) {
		log.debug("getting Branddis instance with id: " + id);
		try {
			Branddis instance = (Branddis) hibernateTemplate.get(
					"molab.main.java.entity.Branddis", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<Object[]> findAvgBranddis(String monid, String start, String finish) {
		log.debug("finding avg all brand from Branddis with monid: " + monid
				+ ", start: " + start + ", finish: " + finish);
		try {
			DetachedCriteria criteria = DetachedCriteria.forClass(Branddis.class);
			criteria.add(Restrictions.eq("id.monid", monid))
				.add(Restrictions.between("id.time", Date.valueOf(start), Date.valueOf(finish)));
	...
	/**
	 * @param monidList format is monid,monid,... */
	public List<Object[]> findAvgBranddisForLarge(String monidList, String start, String finish) {
		log.debug("finding avg all brand from Branddis with monid: " + monidList
				+ ", start: " + start + ", finish: " + finish);
		try {
			DetachedCriteria criteria = DetachedCriteria.forClass(Branddis.class);
			criteria.add(Restrictions.in("id.monid", monidList.split(",")))
				.add(Restrictions.between("id.time", Date.valueOf(start), Date.valueOf(finish)));
			ProjectionList projection = Projections.projectionList();
			projection.add(Projections.avg("counter")).add(Projections.avg("apple"))
				.add(Projections.avg("samsung")).add(Projections.avg("nokia"))
