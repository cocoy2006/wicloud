 ...
 * 
 * @see molab.main.java.entity.Menu
...
public class MenuDAO extends BaseHibernateDAO {
	...
	public Menu findById(java.lang.Integer id) {
		...
		try {
			Menu instance = (Menu) hibernateTemplate.get(
					"molab.main.java.entity.Menu", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<Menu> findByUsername(String username) {
		log.debug("finding all Menu instances");
		try {
			String hql = "from Menu where id in (select menus from Role where name=(select userType from Userinfo where userName='%s'))";
			return hibernateTemplate.find(String.format(hql, username));
		} catch (RuntimeException re) {
			log.error("find all failed", re);
			throw re;
		}
	...
	public Menu merge(Menu detachedInstance) {
		...
		try {
			Menu result = (Menu) hibernateTemplate.merge(detachedInstance);
