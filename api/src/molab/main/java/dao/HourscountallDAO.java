...
import molab.main.java.entity.Hourscountall;
...
@Repository
public class HourscountallDAO extends BaseHibernateDAO<Hourscountall> {
	private static final Logger log = LoggerFactory
			.getLogger(HourscountallDAO.class);

	@Autowired
	private HibernateTemplate hibernateTemplate;
	
	public Object[] findAll_(String monid, String start, String finish) {
		log.debug("finding record from Hourscountall with monid: " + monid
		...
		try {
			...
			String queryString = "SELECT sum(h1),sum(h2),sum(h3),sum(h4),sum(h5),sum(h6),sum(h7),sum(h8),"
					...
					+ "FROM Hourscountall where monid=? and mondate between ? and ?";
